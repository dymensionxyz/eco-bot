...
func NewBondingCurve(m, n, c sdk.Dec) BondingCurve {
	return BondingCurve{
		M: m,
		N: n,
		C: c,
...
func (lbc BondingCurve) ValidateBasic() error {
	...
	if lbc.N.GT(sdk.NewDec(MaxNValue)) {
...
func (lbc BondingCurve) SpotPrice(x math.Int) sdk.Dec {
...
func (lbc BondingCurve) TokensApproximation(startingX, spendTokens sdk.Dec) (sdk.Dec, int, error) {
	// Define the function we're trying to solve: f(x) = Integral(startingX + x) - Integral(startingX) - spendAmt
	f := func(x sdk.Dec) sdk.Dec {
	...
	epsilonDec := sdk.NewDecWithPrec(1, epsilonPrecision)
	for i := 0; i < maxIterations; i++ {
		...
		if fPrimex.IsZero() {
			return sdk.Dec{}, i, fmt.Errorf("division by zero")
		}
		...
		if startingX.Add(x).LT(sdk.OneDec()) {
			x = sdk.OneDec()
		}
	}
	return sdk.Dec{}, maxIterations, fmt.Errorf("solution did not converge")
}

// SpotPrice returns the spot price at x
func (lbc BondingCurve) spotPriceInternal(x sdk.Dec) sdk.Dec {
	xDec := BigDecFromSDKDec(x)
	nDec := BigDecFromSDKDec(lbc.N)
	mDec := BigDecFromSDKDec(lbc.M)
...
func (lbc BondingCurve) integral(x sdk.Dec) sdk.Dec {
	xDec := BigDecFromSDKDec(x)
	mDec := BigDecFromSDKDec(lbc.M)
	cDec := BigDecFromSDKDec(lbc.C)

	nPlusOne := BigDecFromSDKDec(lbc.N.Add(sdk.NewDec(1)))

	var xPowNplusOne BigDec
	if xDec.LT(OneDec()) {
		xPowNplusOne = ZeroDec()
	} else {
...
// c: constant term
// M = (VAL - C * T) * (N + 1) / T^(N+1)
func CalculateM(val, t, n, c sdk.Dec) sdk.Dec {
	valBig := BigDecFromSDKDec(val)
	tBig := BigDecFromSDKDec(t)
	nBig := BigDecFromSDKDec(n)
	cBig := BigDecFromSDKDec(c)

	// Calculate N + 1
	nPlusOne := nBig.Add(OneDec())
	...
	// Convert back to sdk.Dec and return
	return m.SDKDec()
}
...
func ScaleFromBase(x math.Int, precision int64) sdk.Dec {
	return sdk.NewDecFromIntWithPrec(x, precision)
}

// Scales x from the decimal scale to it's base denomination (e.g 1.5 to 1500000000000000)
func ScaleToBase(x sdk.Dec, precision int64) math.Int {
	scaleFactor := math.NewIntWithDecimal(1, int(precision))
	return x.MulInt(scaleFactor).TruncateInt()
}

// checkPrecision checks if a sdk.Dec has at most MaxPrecision decimal places
func checkPrecision(d sdk.Dec) bool {
	// Multiply by 10^MaxPrecision and check if it's an integer
	multiplied := d.Mul(sdk.NewDec(10).Power(uint64(MaxNPrecision)))
